...
class ElevationMap(Shape):
  ...
  def clashTest(self, px, py, pz, rad):
    ...
    * the amount of overlap at the x,z location
    ...
    f = self.buf[0].element_array_buffer # alias to faces array for readability!
    v = self.buf[0].array_buffer # alias to vertices array for readability!
    nVec = self.buf[0].element_normals # alias to normals for faces
    p0 = [px, py, pz] - v[f[:,0],:3] - self.unif[0:3]  # vector from 1st corner to point
    p1 = [px, py, pz] - v[f[:,1],:3] - self.unif[0:3]  # ditto 2nd and 3rd corner
    p2 = [px, py, pz] - v[f[:,2],:3] - self.unif[0:3]  # adjusted for location of map
    ...
    a0 = np.einsum('...j,...j', p0, np.cross(nVec, v[f[:,1],:3] - v[f[:,0],:3]))
    a1 = np.einsum('...j,...j', p1, np.cross(nVec, v[f[:,2],:3] - v[f[:,1],:3]))
    a2 = np.einsum('...j,...j', p2, np.cross(nVec, v[f[:,0],:3] - v[f[:,2],:3]))
