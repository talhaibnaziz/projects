...
struct data{
    ...
    bool operator <(const data &yoo)const{
        return l / base == yoo.l / base ? r < yoo.r : l / base < yoo.l / base;
    }
}Q[N];
...
void dfs(int x, int F, LL d = 0) {
    L[x] = L[F] + 1;
    LCA[x][0] = F;
    S[x] = d;
    for(int i = 0; i < G[x].size(); i++) if(G[x][i] != F) dfs(G[x][i], x, d + C[x][i]);
}

int find_LCA(int x, int y) {
    if(L[x] < L[y]) swap(x, y);
    int log = log2(L[x]);
    for(int i = log; i >= 0; i--) if(L[x] - (1 << i) >= L[y]) x = LCA[x][i];
    if(x == y) return x;
    for(int i = log; i >= 0; i--) if(LCA[x][i] && LCA[x][i] != LCA[y][i]) {
        x = LCA[x][i];
        y = LCA[y][i];
    }
    return LCA[x][0];
...
void built_LCA() {
    ...
    L[0] = -1;
    dfs(1, 0);
    for(int j = 1; (1 << j) <= n; j++) for(int i = 1; i <= n; i++) if(LCA[i][j - 1]) LCA[i][j] = LCA[ LCA[i][j - 1] ][j - 1];
}
...
int main()
{
    while(scanf("%d", &n) != EOF && n) {
        ...
        for(int i = 2; i <= n; i++) {
            int dad; scanf("%d", &dad); dad++;
            int dis; scanf("%d", &dis);
            G[dad].pb(i); C[dad].pb(dis);
            G[i].pb(dad); C[i].pb(dis);
        }
        built_LCA();
        scanf("%d", &qr);
        vL res;
